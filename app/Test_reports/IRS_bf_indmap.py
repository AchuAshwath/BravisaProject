...
if os.name == 'nt':
  ...
  filepath = os.path.join(my_path, "irs-history/")
...
class IRS:
  ...
  def get_btt_list(self,conn, date):
    ...
    if date <= datetime.date(2018,12,27):
      ...
      btt_list_query = 'SELECT * FROM public."BTTList"  WHERE "BTTDate" = \''+ BTT_back + '\';'
        ...
        btt_list_query = """
        SELECT * FROM "BTTList" WHERE "BTTDate" >= %s AND "BTTDate" < %s;
        """
        btt_list = pd.read_sql_query(btt_list_query, con=conn, params=(BTT_back, BTT_next))
  ...
  def get_ohlc_data(self,conn, date):
    ...
    ohlc_sql = """SELECT * FROM "OHLC" WHERE "Date" = %s AND "CompanyCode" IS NOT NULL;"""
    ohlc = pd.read_sql_query(ohlc_sql, con=conn, params=(date,))
  ...
  def calc_change_rate(self,btt_merge, conn, date):
    ...
    ORDER BY "Date" DESC
    """
    ohlc_prev = pd.read_sql_query(ohlc_prev_sql, con=conn, params=(date, week_back))
  ...
  def get_background_info(self,btt_merge, conn):
    ...
    backgroundinfo_sql = """SELECT "IndustryCode", "CompanyCode" FROM "BackgroundInfo";"""
    backgroundinfo = pd.read_sql_query(backgroundinfo_sql, con=conn)
  ...
  def get_industry_data(self,ohlc_backgroundinfo, conn):
    ...
    industry_mapping_sql = """
    SELECT "IndustryCode", "Sector", "Industry", "SubSector", "SectorIndexName", "SubSectorIndexName", "IndustryIndexName"
    FROM "IndustryMapping";
    """
    industry_mapping = pd.read_sql_query(industry_mapping_sql, con=conn)
  ...
  def calc_free_float(self,industry_merge, conn):
    ...
    """
    shareholding = pd.read_sql_query(shareholding_query, con=conn)
  ...
  def industry_ttm(self,industry_list, conn, date):
    ...
    WHERE "YearEnding" <= %s
    GROUP BY "CompanyCode"
    ORDER BY "YearEnding" DESC
    """
    ttm_list = pd.read_sql_query(ttm_sql, con=conn, params=(date,))
    ...
    ORDER BY "YearEnding" DESC
    """
    ttm_back_list = pd.read_sql_query(ttm_back_sql, con=conn, params=(year_back,))
  ...
  def insert_industry_list(self,industry_list, conn, cur, date):
    ...
    industry_list.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator='\r')
    exportfile.close()

    
    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "IndustryList"
  ...
  def get_industry_list_backdate(self,conn, date):
    ...
    sql = """SELECT * FROM "IndustryList" WHERE "GenDate" <= %s;"""
    print("SQL QUERY: ", sql)
    master_list = pd.read_sql_query(sql, con=conn, params=(str(date),))
  ...
  def calc_sector_divisor(self,master_list, sector_divisor, conn, date):
    ...
    ORDER BY "DATE" DESC
    """
    indexhistory = pd.read_sql_query(indexhistory_sql, con=conn, params=(date,))

    divisor_backdate_sql = """
    SELECT * FROM "SectorDivisor"
    WHERE "Date" < %s
    GROUP BY "IndexName"
    ORDER BY "Date" DESC
    """
    divisor_backdate = pd.read_sql_query(divisor_backdate_sql, con=conn, params=(date,))

    sql = """
    SELECT "SectorIndexName", "Sector", SUM("FF_Close") AS ff_close_sum, SUM("OS") AS os_sum
    FROM "IndustryList"
    WHERE "GenDate" = %s AND "SectorIndexName" IS NOT NULL
    GROUP BY "SectorIndexName", "Sector"
    """
    sector_divisor_list = pd.read_sql_query(sql, con=conn, params=(date,))
  ...
  def calc_subsector_divisor(self,master_list, subsector_divisor, conn, date):
    ...
    sql = """
    SELECT "SubSectorIndexName", "SubSector", SUM("FF_Close") AS ff_close_sum, SUM("OS") AS os_sum
    FROM "IndustryList"
    WHERE "GenDate" = %s AND "SubSectorIndexName" IS NOT NULL
    GROUP BY "SubSectorIndexName", "SubSector"
    """
    subsector_divisor_list = pd.read_sql_query(sql, con=conn, params=(date,))
  ...
  def calc_industry_divisor(self,master_list, industry_divisor, conn, date):
    ...
    sql = """
    SELECT "IndustryIndexName", "Industry", SUM("FF_Close") AS ff_close_sum, SUM("OS") AS os_sum
    FROM "IndustryList"
    WHERE "GenDate" = %s AND "IndustryIndexName" IS NOT NULL
    GROUP BY "IndustryIndexName", "Industry"
    """
    industry_divisor_list = pd.read_sql_query(sql, con=conn, params=(date,))
  ...
  def insert_sector_divisor(self,merge_sector_divisor, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    merge_sector_divisor.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator='\r')
    exportfile.close()
    
    
    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "SectorDivisor"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def insert_subsector_divisor(self,merge_subsector_divisor, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    merge_subsector_divisor.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator='\r')
    exportfile.close()

    
    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "SubSectorDivisor"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def insert_industry_divisor(self,merge_industry_divisor, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    merge_industry_divisor.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator='\r')
    exportfile.close()

    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "IndustryDivisor"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def gen_sector_index(self,conn, date):
    ...
    WHERE "Date" = %s
    """
    sector_divisor = pd.read_sql_query(sector_divisor_sql, con=conn, params=(date,))

    sector_ff_ohlc_sql = """
    SELECT "SectorIndexName", "Sector", SUM("OS_Close") AS sum_os_close, SUM("FF_Open") AS ff_open_sum, 
    SUM("FF_High") AS ff_high_sum, SUM("FF_Low") AS ff_low_sum, SUM("FF_Close") AS ff_close_sum, 
    SUM("Volume") AS sum_vol, SUM("OS") AS os_sum, SUM("PAT") AS earnings, SUM("Equity") AS sum_equity, 
    SUM("prev_pat") AS prev_earnings, SUM("prev_equity") AS prev_equity, COUNT(*) AS company_count
    FROM "IndustryList"
    WHERE "GenDate" = %s AND "SectorIndexName" IS NOT NULL
    GROUP BY "SectorIndexName", "Sector"
    """
    sector_ff_ohlc = pd.read_sql_query(sector_ff_ohlc_sql, con=conn, params=(date,))
  ...
  def insert_sector_index(self,sector_index, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    sector_index.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator = '\r')
    exportfile.close()

    
    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "SectorIndexList"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def gen_subsector_index(self,conn, date):
    ...
    subsector_divisor_sql = """
    SELECT * FROM "SubSectorDivisor"
    WHERE "Date" = %s
    """
    subsector_divisor = pd.read_sql_query(subsector_divisor_sql, con=conn, params=(date,))

    subsector_ff_ohlc_sql = """
    SELECT "SubSectorIndexName", "SubSector", SUM("OS_Close") AS sum_os_close, SUM("FF_Open") AS ff_open_sum, 
    SUM("FF_High") AS ff_high_sum, SUM("FF_Low") AS ff_low_sum, SUM("FF_Close") AS ff_close_sum, 
    SUM("Volume") AS sum_vol, SUM("OS") AS os_sum, SUM("PAT") AS earnings, SUM("Equity") AS sum_equity, 
    SUM("prev_pat") AS prev_earnings, SUM("prev_equity") AS prev_equity, COUNT(*) AS company_count
    FROM "IndustryList"
    ...
    GROUP BY "SubSectorIndexName", "SubSector"
    """
    subsector_ff_ohlc = pd.read_sql_query(subsector_ff_ohlc_sql, con=conn, params=(date,))
  ...
  def insert_subsector_index(self,subsector_index, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    subsector_index.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator = '\r')
    exportfile.close()

    
    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "SubSectorIndexList"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def gen_industry_index(self,conn, date):
    ...
    industry_divisor_sql = """
    SELECT * FROM "IndustryDivisor"
    WHERE "Date" = %s
    """
    industry_divisor = pd.read_sql_query(industry_divisor_sql, con=conn, params=(date,))

    industry_ff_ohlc_sql = """
    SELECT "IndustryIndexName", "Industry", SUM("OS_Close") AS sum_os_close, SUM("FF_Open") AS ff_open_sum, 
    SUM("FF_High") AS ff_high_sum, SUM("FF_Low") AS ff_low_sum, SUM("FF_Close") AS ff_close_sum, 
    SUM("Volume") AS sum_vol, SUM("OS") AS os_sum, SUM("PAT") AS earnings, SUM("Equity") AS sum_equity, 
    SUM("prev_pat") AS prev_earnings, SUM("prev_equity") AS prev_equity, COUNT(*) AS company_count
    FROM "IndustryList"
    ...
    GROUP BY "IndustryIndexName", "Industry"
    """
    industry_ff_ohlc = pd.read_sql_query(industry_ff_ohlc_sql, con=conn, params=(date,))
  ...
  def insert_industry_index(self,industry_index, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    industry_index.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator = '\r')
    exportfile.close()

    
    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "IndustryIndexList"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def calc_rank_index(self,sector_index, subsector_index, industry_index, conn, date):
    ...
    indexlist_history_sql = """
    SELECT DISTINCT ON ("TICKER") * FROM "IndexHistory"
    WHERE "DATE" <= %s
    ORDER BY "TICKER", "DATE" DESC
    """
    index_history = pd.read_sql_query(indexlist_history_sql, con=conn, params=(six_month_back,))
  ...
  def calc_pe_high_low(self,irs_list, conn, date):
    ...
    irs_back_sql = """
    SELECT DISTINCT ON ("IndexName") * FROM "Reports"."IRS"
    WHERE "GenDate" < %s
    ORDER BY "IndexName", "GenDate" DESC
    """
    irs_back = pd.read_sql_query(irs_back_sql, con=conn, params=(date,))
  ...
  def insert_irs(self,irs_list, conn, cur, date):
    ...
    exportfile = open(exportfilename,"w")
    irs_list.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator = '\r')
    exportfile.close()

    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "Reports"."IRS"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
    ...
    exportfile = open(exportfilename,"w")
    index_history.to_csv(exportfile, header=True, index=False, float_format="%.2f", line_terminator = '\r')
    exportfile.close()

    load_data_sql = """
    LOAD DATA INFILE %s INTO TABLE "IndexHistory"
    FIELDS TERMINATED BY ',' 
    ENCLOSED BY '"'
    LINES TERMINATED BY '\n'
    IGNORE 1 ROWS;
  ...
  def insert_irs_history(self,conn, cur):
    ...
    sector_csv =  filepath + 'sectorindex.csv'
    sector_history = pd.read_csv(sector_csv, encoding = 'latin1')

    subsector_csv = filepath + 'subsectorindex.csv'
    subsector_history = pd.read_csv(subsector_csv, encoding = 'latin1')

    industry_csv = filepath + 'industryindex.csv'
  ...
  def pe_update(self,conn, cur, date):
    ...
    irs_sql = """
    SELECT * FROM "Reports"."IRS"
    WHERE "GenDate" = %s
    """
    irs = pd.read_sql_query(irs_sql, con=conn, params=(date,))
