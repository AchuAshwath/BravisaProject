...
class CombinedRank():
    ...
    def get_all_rs(self, date, conn):
        ...
        prs_sql = "SELECT `CompanyCode`, `Combined Strength`, `Value Average` from `Reports`.`PRS` where `Date` = %s;"
        prs = pd.read_sql_query(prs_sql, con=conn, params=(date,))

        if not(prs.empty):

            ers_sql = "SELECT `CompanyCode`, `Ranking` from `Reports`.`EPS` where `EPSDate` = %s;"
            ers = pd.read_sql_query(ers_sql, con=conn, params=(date,))

            rrs_sql = "SELECT `CompanyCode`, `SMR Rank` from `Reports`.`SMR` where `SMRDate` = %s;"
            rrs = pd.read_sql_query(rrs_sql, con=conn, params=(date,))

            frs_sql = "SELECT `CompanyCode`, `MFRank` from `Reports`.`FRS-MFRank` where `Date` = %s;"
            frs = pd.read_sql_query(frs_sql, con=conn, params=(date,))

            irs_sql = "SELECT distinct il.`CompanyCode`, irs.`Rank` from `IndustryList` il \
                        left join `Reports`.`IRS` irs \
                        on il.`IndustryIndexName` = irs.`IndexName` \
                        where il.`GenDate` = %s \
                        and irs.`GenDate` = %s;"
            irs = pd.read_sql_query(irs_sql, con=conn, params=(date, date))
    ...
    def merge_background(self, StockRS, conn):
        ...
        background_sql = "SELECT * FROM `BackgroundInfo`;"
        background_info = pd.read_sql_query(background_sql, con=conn))
    ...
    def insert_combined_rs(self, StockRS, conn, cur, date):
        ...
        StockRS.to_csv(exportfile, header=True, index=False,
                       float_format="%.2f", line_terminator='\r')
        exportfile.close()

        load_data_sql = """
        LOAD DATA INFILE %s INTO TABLE `Reports`.`CombinedRS`
