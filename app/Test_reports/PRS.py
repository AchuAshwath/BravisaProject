...
class PRS:
	...
	def fetch_btt_prs(self, curr_date, conn):
		...
		btt_sql = """
		SELECT DISTINCT ON (`CompanyCode`) `ISIN`, `NSECode`, `BSECode`, `CompanyName`, `CompanyCode`
		FROM `BTTList`
	...
	def fetch_ohlc_prs(self, curr_date, bttlist, conn):
		...
		WHERE `Date` = %s
		"""
		ohlc_company = pd.read_sql_query(sql, conn, params=(today_date,))

		bttlist = pd.merge(bttlist, ohlc_company, on=['NSECode', 'BSECode'], how='left')
		
		bttlist.rename(columns={'CompanyCode_x': 'CompanyCode','ISIN_x': 'ISIN'}, inplace=True)
		columns_to_remove = ["CompanyCode_y", "ISIN_y", "Company"]
	...
	def fetch_highlow_prs(self, curr_date, bttlist, conn):
		...
		"""
		ohlc_full = pd.read_sql_query(sql, conn, params=(today_date, year_back))
	...
	def value_average(self,curr_date,bttlist, conn):
		...
		WHERE `Date` <= %s AND `Date` >= %s
		"""
		ohlc_full = pd.read_sql_query(sql, conn, params=(today_date, eightyday_back))
	...
	def getclosestdate_ohlc(self,companylist,s_date):
		...
		if not (companylist.loc[mask_0].empty):
			...
			return companylist.loc[mask_0, "Close"].item()
		else:
			if not (companylist.loc[mask_b_1].empty):
				# print("Not Empty at -1")
				return companylist.loc[mask_b_1, "Close"].item()
			else:
				if not (companylist.loc[mask_b_2].empty):
					# print("Not Empty at -2")
					return companylist.loc[mask_b_2, "Close"].item()
				else:
					if not (companylist[mask_1].empty):
						# print("Not Empty at 1")
						return companylist.loc[mask_1, "Close"].item()
					else:
						if not (companylist.loc[mask_b_3].empty):
							# print("Not Empty at -3")
							return companylist.loc[mask_b_3, "Close"].item()
						else:
	...
	def merge_pe_prs(self,bttlist, conn, date):
		...
		WHERE `GenDate` = %s
		"""
		pe = pd.read_sql_query(pe_sql, con=conn, params=(date,))
	...
	def insert_nhnl(self,bttlist, conn):
		...
		load_data_sql = """
		LOAD DATA INFILE %s INTO TABLE `Reports`.`NewHighNewLow`
	...
	def insert_prs(self,bttlist, conn):
		...
		exportfile = open(exportfilename, "w")
		bttlist.to_csv(exportfile, header=True, index=False,float_format="%.2f", line_terminator='\r')
		exportfile.close()

		load_data_sql = """
		LOAD DATA INFILE %s INTO TABLE `Reports`.`PRS`
		FIELDS TERMINATED BY ',' 
		ENCLOSED BY '"'
		LINES TERMINATED BY '\n'
		IGNORE 1 ROWS;
