...
from lib.check_helper import Check_Helper
import psycopg2
import pandas as pd
import numpy as np
import shutil
...
if os.name == 'nt':
	my_path = os.path.abspath(os.path.dirname('C:\\Users\\dsram\\BravisaLocalDeploy\\BravisaFiles\\'))
	# my_path = os.path.abspath(os.path.dirname('C:\\Users\\dsram\\Downloads\\Bravisa_ResultsData_30302022\\'))
...
class FB_Insert:
	...
	def __init__(self):
		self.engine = create_engine('postgresql+psycopg2://postgres:bravisatempletree@localhost:5432/BravisaDB')
	...
	def insert_annual_meetings(self, conn,cur,fbname):
		...
		if os.path.isfile(file_to_check):
			try:
				...
				table.to_csv(exportfile, header=True, index=False, line_terminator='\r')
				exportfile.close()
				...
				print("Error in insert_annual_meetings()", flush = True)
				conn.rollback()
		else:
	...
	def insert_backgroundinfo(self, conn, cur, fbname):
		""" Insert the Backgroundinfo data into database

		Operation:
			Set the path for csv file, and fetch the data from BackgroundInfo.csv'file.
			and delete the data based on key column CompanyCode, and 
			export the executed data into 'BackgroundInfoExport.csv' file and insert into BackgroundInfo Table.
		"""
		fb_name = fbname
		fb_csv_path = os.path.join(file_path, fb_name +'\\')	
		fb_csv_file = fb_csv_path + 'BackgroundInfo.csv'
		
		file_to_check = file_path + fb_name + '\\' + 'BackgroundInfo.csv'
		...
		if os.path.isfile(file_to_check):
			try:
				...
				table['BSECode'].astype('int32').dtypes
	...
	def insert_boardmeetings(self, conn, cur, fbname):
		""" Insert the Board Meeting data into database

		Operation:
			Set the path for csv file, and fetch the data from BoardMeeting.csv file.
			delete the data based on key column,CompanyCode and BoardMeetDate
			and export the executed data into 'BoardMeetingsExport.csv' file 
			and insert into BoardMeetings Table.
		"""
		fb_name = fbname
		fb_csv_path = os.path.join(file_path, fb_name +'\\')	
		fb_csv_file = fb_csv_path + 'BoardMeetings.csv'
		
		file_to_check = file_path + fb_name + '\\' + 'BoardMeetings.csv'
		...
		if os.path.isfile(file_to_check):
			try:
				...
				#Update Logic - Deletes based on key columns 
				print("Executing delete logic", flush = True)
				table_to_delete = table.groupby(['CompanyCode', 'BoardMeetDate'], as_index=False).count()
				self.logfile.write("\tIn function: " + str(fb_csv_file.split("\\")[-1].split(".")[0]) + 
							"\t\tTotal Loops:" + str(int(table_to_delete.shape[0])) + "\t\tFor file: " + str(fbname) + "\n")
				self.logfile.flush()
				for index,row in table_to_delete.iterrows():
					...
					# 		"\t\tTotal Loops:" + str(int(table_to_delete.shape[0])) + "\t\tFor file: " + str(fbname) + "\n")
					# self.logfile.flush()
					sql = sql = 'DELETE FROM public."BoardMeetings" WHERE "CompanyCode"=\'' +str(row['CompanyCode'])+ '\' AND "BoardMeetDate"=\'' + str(row['BoardMeetDate'])+'\';'   
					cur.execute(sql)
					conn.commit()
				

				print("Inserting Data into BoardMeetings", flush = True)
				exportfilename = "BoardMeetingsExport.csv"
				exportfile = open(exportfilename,"w", encoding="utf-8")
				table.to_csv(exportfile, header=True, index=False, line_terminator='\r')
				exportfile.close()
				
				copy_sql = """
					COPY "public"."BoardMeetings" FROM stdin WITH CSV HEADER
					DELIMITER as ','
					"""
				with codecs.open(exportfilename, "r",encoding='utf-8', errors='ignore') as f:
					cur.copy_expert(sql=copy_sql, file=f)
	...
	def insert_capitalstructure(self, conn, cur, fbname):
		""" Insert the Capital Structure data into database

		Operation:
			Set the path for csv file, and fetch the data from CapitalStructure.csv file.
			delete the data based on key column,companycode, FromYear and ToYear
			and Export data into 'CapitalStructureExport.csv' file and insert into CapitalStructur Table.
		"""
		fb_name = fbname
		fb_csv_path = os.path.join(file_path, fb_name +'\\')	
		fb_csv_file = fb_csv_path + 'CapitalStructure.csv'
		
		file_to_check = file_path + fb_name + '\\' + 'CapitalStructure.csv'
		...
		if os.path.isfile(file_to_check):
			try:
				...
				for index,row in table_to_delete.iterrows():
					...
					# 		"\t\tTotal Loops:" + str(int(table_to_delete.shape[0])) + "\t\tFor file: " + str(fbname) + "\n")
					# self.logfile.flush()
					sql = 'DELETE FROM public."CapitalStructure" WHERE "companycode"=\'' +str(row['companycode'])+ '\' AND "FromYear"=\'' + str(row['FromYear'])+'\' AND "ToYear"=\''+ str(row['ToYear'])+'\';'   
					cur.execute(sql)
					conn.commit()
				...
				print("Error in insert_capitalstructure()", flush = True)
				conn.rollback()
		else:
			print("File not found: " + fb_csv_file, flush = True)
	...
	def insert_keyexecutives(self, conn, cur,fbname ):
		""" Insert the Key Executives data into database
		...
		fb_name = fbname
		fb_csv_path = os.path.join(file_path, fb_name +'\\')  
		fb_csv_file = fb_csv_path + 'KeyExecutives.csv'
		
		file_to_check = file_path + fb_name + '\\' + 'KeyExecutives.csv'
		...
		if os.path.isfile(file_to_check):
			try:
				...
				for index,row in table_to_delete.iterrows():
					...
					# 		"\t\tTotal Loops:" + str(int(table_to_delete.shape[0])) + "\t\tFor file: " + str(fbname) + "\n")
					# self.logfile.flush()
					sql = 'DELETE FROM public."KeyExecutives"  WHERE "Companycode" =\'' +str(row['Companycode']) + '\' ;'
					##print(sql, flush = True)
					cur.execute(sql)
					conn.commit()
				...
				print("Error in insert_keyexecutives()", flush = True)
				conn.rollback()
	...
	def insert_managementteam(self, conn, cur, fbname ):
		...
		if os.path.isfile(file_to_check):
			try:
				...
				print("Error in insert_managementteam()", flush = True)
				conn.rollback()
	...
	def insert_registrars(self, conn, cur,fbname ):
		...
		if os.path.isfile(file_to_check):
			try:
				...
				print("Error in insert_registrars()", flush = True)
				conn.rollback()
	...
	def insert_schemefundmanagerprofile(self, conn, cur,fbname ): 
		...
		if os.path.isfile(file_to_check):
			try:
				...
				print("Error in insert_schemefundmanagerprofile()", flush = True)
				conn.rollback()
	...
	def insert_schemelocations(self, conn, cur,fbname ): 
		...
		if os.path.isfile(file_to_check):
			try:
				...
				print("Error in insert_schemelocations()", flush = True)
				conn.rollback()
		else:
			print("File not found: " + fb_csv_file, flush = True)

	def insert_schememaster(self, conn, cur, fbname ):
		""" Insert the SchemeMaster data into database

		Operation:
			Set the path for csv file, and fetch the data from SchemeMaster.csv file.
			delete the data based on key column SchemeCode and SchemePlanCode,
			export executed data into 'SchemeMasterExport.csv' file 
			and insert into SchemeMaster Table.
		"""
		...
		file_to_check = file_path + fb_name + '\\' + 'SchemeMaster.csv'
		...
		if os.path.isfile(file_to_check):
			...
			try:
				...
				'''
				table.loc[table['LaunchDate'] == '""', 'LaunchDate'] = ""
				table.loc[table['OpenDate'] == '""', 'OpenDate'] = ""
				table.loc[table['CloseDate'] == '""', 'CloseDate'] = ""
				table.loc[table['RedemptionDate'] == '""', 'RedemptionDate'] = ""
				'''
				...
				# table['SIP_Min_Invt'] = table['SIP_Min_Invt'].fillna('Nil')
				table['ExitLoad_WithSlab_Desc'] = table['ExitLoad_WithSlab_Desc'].replace("\n", " ")
				table.to_csv(exportfile, header=True, index=False, line_terminator='\n')
				exportfile.close()
				print("Added data to csv", flush = True)
				tabletwo = pd.read_csv(exportfilename, encoding='cp1252')
				# for i, up in tabletwo['ModifiedDate'].iteritems():
				# 	print(up)
	...
	def insert_bulkdeals(self, conn, cur, fbname):
		...
		if os.path.isfile(file_to_check):
			try:
				...
				print("Error in insert_bulkdeals()", flush = True)
				conn.rollback()
