...
def update_ohlc(split, bonus, conn, cur, date):
    ...
    if not(split.empty):
        ...
        update_sql = 'UPDATE public."OHLC" ohlc \
					  SET \
						"Open" = "Open" / split_val, \
						"High" = "High" / split_val, \
						"Low" = "Low" / split_val, \
						...
						FROM (SELECT "OldFaceValue"/"NewFaceValue" AS split_val, "CompanyCode", "XSDate"  FROM public."Splits" 	WHERE "XSDate" = \'' + date+'\' ) AS splits \
    ...
    if not(bonus.empty):
        ...
        update_sql = 'UPDATE public."OHLC" ohlc \
					  ...
					  FROM (SELECT "RatioOfferred"/"RatioExisting" AS bonus_val, "CompanyCode", "XBDate" FROM public."Bonus" WHERE "XBDate" = \''+date+'\' ) AS bonus \
...
def update_shareholding(split, bonus, conn, cur, date):
    ...
    if not(split.empty):
        ...
        update_sql = 'UPDATE public."ShareHolding" sh \
					  SET \
					 ...
					 FROM (SELECT "OldFaceValue"/"NewFaceValue" AS split_val, "CompanyCode", "XSDate"  FROM public."Splits" WHERE "XSDate" = \''+date+'\') AS split \
						WHERE sh."CompanyCode" = split."CompanyCode" \
						AND sh."SHPDate" = (SELECT MAX("SHPDate") FROM public."ShareHolding" where "CompanyCode" = split."CompanyCode") \
        ...
        update_sql = 'UPDATE public."ShareHolding" sh \
            ...
            "Total" = "Total" * (1+bonus_val)   \
					 FROM  (SELECT "RatioOfferred"/"RatioExisting" AS bonus_val, "CompanyCode", "XBDate" FROM public."Bonus"  WHERE "XBDate" = \''+date+'\') AS bonus \
					 	...
					 	AND sh."SHPDate" = (SELECT MAX("SHPDate") FROM public."ShareHolding" where "CompanyCode" = bonus."CompanyCode") \
