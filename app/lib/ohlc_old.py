...
def fetch_bse(conn, cur, curr_date):
    ...
    download_date = curr_date.strftime("%d%m%y")
    # print("download date\t",download_date)

    print("\n\nBSE Fetch invoked ....")

    csv_file = filepath+ 'EQ_ISINCODE_' + download_date + '\\'+ 'EQ_ISINCODE_' + download_date + '.CSV'
...
def fetch_nse(conn, cur, curr_date):
    ...
    download_date = curr_date.strftime("%d%b%Y").upper()

    csv_file = filepath+"cm"+download_date+"bhav.csv\\"+"\\cm"+download_date+"bhav.csv"
