...
class FB_Insert:
	...
	def insert_consolidatedquarterlyresults(self, conn, cur, fbname):
		...
		"""
		fbdate =fbname[6:10] + '-' + fbname[4:6] + '-' + fbname[2:4]
		fbdate = pd.to_datetime(fbdate)

		fb_name = fbname 
		fb_csv_path = os.path.join(file_path, fb_name +'\\')	
		fb_csv_file = fb_csv_path + 'ConsolidatedQuarterlyResults.csv'

		missing_dir = 'C:\\BravisaProject\\app\\missing'
		...
		if os.path.isfile(file_to_check):
			try:
				# table = pd.read_csv(fb_csv_file, engine='python')
				concat_MissingConsolidatedQuarterlyResults = MissingConsolidatedQuarterlyResults[MissingConsolidatedQuarterlyResults['ModifiedDate'].dt.date == fbdate.date()]
	...
	def insert_quarterlyresults(self, conn, cur,fbname ):
		...
		if os.path.isfile(file_to_check):
			try:
				# table = pd.read_csv(fb_csv_file, engine='python')
				concat_MissingQuarterlyResults = MissingQuarterlyResults[MissingQuarterlyResults['ModifiedDate'].dt.date == fbdate.date()]
	...
	def fb_insert_01(self, fbname,conn, cur):
		...
		try: 
			...
			Check_Helper().check_FBpath(file_path)
			# date = fbname[2:-2]
			...
			print("File not found: ", e, flush = True)
			return False
	...
	def fb_insert_02(self, fbname,conn,cur):
		...
		try:
			...
			Check_Helper().check_FBpath(fbname)
