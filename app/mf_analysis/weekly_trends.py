# Python script to generate trends data for weekly OHLC
import datetime
import requests
import os.path
import os
...
class WeeklyTrends():
    """ Contains methods to calculate trends data for stocks based on weekly indicator data.
    """
    ...
    def get_indicator_weekly(self, conn, date):
        """ Function to get weekly indicator data for latest date.
            ...
            Returns: 
                indicator_weekly: dataframe containing indicator data for given date. 

            Raises:
                No errors/exceptions. 
        """

        sql = 'SELECT * FROM mf_analysis.indicators_weekly WHERE "gen_date" =  \'' + \
            str(date)+'\';'
        indicator_weekly = sqlio.read_sql_query(sql, con=conn)

        return indicator_weekly

    def get_indicator_weekly_back(self, conn, date):
        """ Function to get weekly indicator date for previous date.
            ...
            Returns: 
                indicator_weekly_back: dataframe containing indicator data for previous date.  

            Raises:
                No errors/exceptions. 
        """

        sql = 'SELECT DISTINCT ON("company_code") * FROM mf_analysis.indicators_weekly \
               WHERE "gen_date" < \''+str(date)+'\' \
               ORDER BY "company_code", "gen_date" DESC;'
        indicator_weekly_back = sqlio.read_sql_query(sql, con=conn)

        return indicator_weekly_back

    def get_trends_weekly(self, indicator_weekly, indicator_weekly_back):
        """ Function to get weekly trends data for latest date. 

            Args:
                indicator_weekly: current week's indicator data. 
                indicator_weekly_back: previous week's indicator data. 

            Returns: 
                trends_weekly: dataframe containing weekly trends data. 

            Raises:
                No errors/exceptions.  
        """

        trends_weekly = self.calc_trends.calc_trends(
            indicator_weekly, indicator_weekly_back)

        return trends_weekly

    def insert_weekly_trends(self, conn, cur, trends_data):
        """ Function to insert Weekly trends data into the DB.

            Args:
                conn: database connection.
                cur: cursor using the connection. 
                trends_data: trends data returned from get_trends_weekly function. 
        ...
        copy_sql = """
            COPY mf_analysis.trends_weekly FROM stdin WITH CSV HEADER
            DELIMITER as ','
            """
