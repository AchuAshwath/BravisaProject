...
class EPS:
    ...
    def quarterly_eps_history_insert(self, conn, logging):
        ...
        for index, row in quarterly_list.iterrows():
            ...
            if eps_prev == np.nan:
                logging.info(f"(Q1)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyResults for the YearEnding={prev_year} doesnt exist or has more than one value.\n\n")
            
            eps_current = row['EPS']

            quarterly_list.loc[index, 'Q1 EPS Growth'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev  !=0 else np.nan
            if quarterly_list.loc[index, 'Q1 EPS Growth'] == np.nan:
                logging.info(f"Q1 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in QuarterlyResults for the YearEnding={prev_year} doesnt exist or has more than one value.\n\n")
            ...
            if sales_prev == np.nan:
                logging.info(f"(Q1)Sales Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in QuarterlyResults for the YearEnding={prev_year} doesnt exist or has more than one value.\n\n")
            ...
            if quarterly_list.loc[index, 'Q1 Sales Growth'] == np.nan:
                logging.info(f"Q1 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales Previous in QuarterlyResults for the YearEnding={prev_year} doesnt exist or has more than one value.\n\n")
            ...
            eps_prev = eps_prev_list.item() if len(eps_prev_list.index) == 1 else np.nan
            if eps_prev == np.nan:
                logging.info(f"(Q2)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyResults for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ...
            if eps_current == np.nan:
                logging.info(f"(Q2)EPS Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyResults for the YearEnding={prev_qtr} doesnt exist or has more than one value.\n\n")

            quarterly_list.loc[index, 'Q2 EPS Growth'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev  !=0 else np.nan
            if quarterly_list.loc[index, 'Q2 EPS Growth'] == np.nan:
                logging.info(f"Q2 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in QuarterlyResults for the YearEnding={prev_year} doesnt exist or has more than one value.\n\n")
            ...
            sales_prev = sales_prev_list.item() if len(sales_prev_list.index) == 1 else np.nan
            if sales_prev == np.nan:
                logging.info(f"(Q2)Sales Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyResults for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ...
            if sales_current == np.nan:
                logging.info(f"(Q2)Sales Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyResults for the YearEnding={prev_qtr} doesnt exist or has more than one value.\n\n")

            quarterly_list.loc[index, 'Q2 Sales Growth'] = ((sales_current-sales_prev)/abs(sales_prev))*100 if sales_prev !=0 else np.nan
            if quarterly_list.loc[index, 'Q2 Sales Growth'] == np.nan:
                logging.info(f"Q2 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in QuarterlyResults for the YearEnding={prev_year} doesnt exist or has more than one value.\n\n")
    ...
    def inset_ttm_history(self, conn,logging):
        """ Insert TTM history
        ...
        while(today1 >= back_date_history):
            ...
            start = time.time()
            ttm = self.ttm_quarterly_list(today1, conn, logging)
            end = time.time()
    ...
    def quarterly_one_eps_sales_growth(self, quarterly_eps_list, conn,today, logging):
        ...
        print('aa1')
        ##########################################
        logging.info("Q1 EPS Sales Growth:")
        ##########################################
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            if len(eps_prev_list.index) > 1:
                ...
                ##########################################
                if eps_prev == np.nan:
                    logging.info(f"(Q1)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS value in QuarterlyEPS for YearEnding - {prev_year_qtr} (1 year before quarter) doesnt exist.\nConsidering Closest quarter.\n\n")
                ##########################################
            ...
            if (math.isnan(eps_prev)):
                ...
                ##########################################
                if eps_prev == np.nan:
                    logging.info(f"(Q1)EPS Prev = NaNIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS value in QuarterlyEPS for the closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
                ...
                ##########################################
                if quarterly_eps_list.loc[index, 'Q1 EPS Growth'] == np.nan:
                    logging.info(f"Q1 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in QuarterlyEPS for closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
            ...
            if quarterly_eps_list.loc[index, 'Q1 EPS Growth'] == np.nan:
                logging.info(f"Q1 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in QuarterlyEPS for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            ...
            if len(sales_prev_list.index) > 1:
                ...
                if sales_prev == np.nan:
                    logging.info(f"(Q1)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales value in QuarterlyEPS for YearEnding - {prev_year_qtr} (1 year before quarter) doesnt exist.\nConsidering Closest quarter.\n\n")
                ##########################################
            if (math.isnan(sales_prev)):
                ...
                ##########################################
                if sales_prev == np.nan:
                    logging.info(f"(Q1)Sales Prev = NaNIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales value in QuarterlyEPS for the closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
                ...
                ##########################################
                if quarterly_eps_list.loc[index, 'Q1 Sales Growth'] == np.nan:
                    logging.info(f"Q1 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales Previous in QuarterlyEPS for closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
            ...
            if quarterly_eps_list.loc[index, 'Q1 Sales Growth'] == np.nan:
                logging.info(f"Q1 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales Previous in QuarterlyEPS for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################

        return quarterly_eps_list


    def consolidated_quarterly_one_eps_sales_growth(self, quarterly_eps_list, conn, today, logging):
        """Calculating the EPS and Sales Growth for current quarter,
        ...
        Return:
          ...
          """
        ##########################################
        logging.info("Consolidated Q1 EPS Sales Growth:")
        ##########################################
        qtr_one_start = self.get_closest_quarter(today).strftime("%Y-%m-%d")
        prev_year =  self.get_year_before_quarter(self.get_closest_quarter(today))
        prev_2year =  self.get_year_before_quarter(prev_year).strftime("%Y-%m-%d")
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            if len(eps_prev_list.index) > 1:
                ...
                if eps_prev == np.nan:
                    logging.info(f"(Q1)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS value in ConsolidatedQuarterlyEPS for YearEnding - {prev_year_qtr} (1 year before quarter) doesnt exist.\nConsidering Closest quarter.\n\n")
                ##########################################
            if (math.isnan(eps_prev)):
                eps_prev_list = quarterly_eps_list.loc[(quarterly_eps_list["CompanyCode"]==row['CompanyCode']) & (quarterly_eps_list["YearEnding"] == prev_year_qtr)]["EPS"]
                # eps_prev = eps_prev_list.item() if len(eps_prev_list.index) != 0 else np.nan
                ...
                if eps_prev == np.nan:
                    logging.info(f"(Q1)EPS Prev = NaNIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS value in ConsolidatedQuarterlyEPS for the closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
                ...
                if quarterly_eps_list.loc[index, 'Q1 EPS Growth'] == np.nan:
                    logging.info(f"Q1 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in ConsolidatedQuarterlyEPS for closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
            ...
            if quarterly_eps_list.loc[index, 'Q1 EPS Growth'] == np.nan:
                logging.info(f"Q1 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS Previous in ConsolidatedQuarterlyEPS for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            ...
            if len(sales_prev_list.index) > 1:
                ...
                if sales_prev == np.nan:
                    logging.info(f"(Q1)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales value in ConsolidatedQuarterlyEPS for YearEnding - {prev_year_qtr} (1 year before quarter) doesnt exist.\nConsidering Closest quarter.\n\n")
                ##########################################
            ...
            if (math.isnan(sales_prev)):
                ...
                if sales_prev == np.nan:
                    logging.info(f"(Q1)Sales Prev = NaNIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales value in ConsolidatedQuarterlyEPS for the closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
                ...
                if quarterly_eps_list.loc[index, 'Q1 Sales Growth'] == np.nan:
                    logging.info(f"Q1 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales Previous in ConsolidatedQuarterlyEPS for closest quarter doesnt exist or has more than one value.\n\n")
                ##########################################
            # q1_f_salesprev.write("CompanyCode: {}\nsales_current: {}\nsales_prev: {}\nlen(sales_prev_list.index): {}\n\n".format(row['CompanyCode'], sales_current, sales_prev, len(sales_prev_list.index)))
            quarterly_eps_list.loc[index, 'Q1 Sales Growth'] = ((sales_current-sales_prev)/abs(sales_prev))*100 if sales_prev  !=0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'Q1 Sales Growth'] == np.nan:
                logging.info(f"Q1 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales Previous in ConsolidatedQuarterlyEPS for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
    ...
    def quarterly_two_eps_sales_growth(self, quarterly_eps_list, conn,today, logging):
        """Calculating the EPS and Sales Growth for current quarter,
        ...
        Return:
         ...
         """
        ##########################################
        logging.info("Q2 EPS Sales Growth:")
        ##########################################
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            if eps_prev == np.nan:
                logging.info(f"(Q2)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyEPS where YearEnding={prev_year_qtr} (1 year before 2nd quarter) doesnt exist or has more than one value.\n\n")
            ##########################################
            ...
            if eps_current == np.nan:
                logging.info(f"(Q2)EPS Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in QuarterlyEPS for YearEnding={prev_qtr} (previous quarter) doesnt exist or has more than one value.\n\n")
                        
            quarterly_eps_list.loc[index, 'Q2 EPS Growth'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev  !=0 else np.nan
            if quarterly_eps_list.loc[index, 'Q2 EPS Growth'] == np.nan:
                    logging.info(f"Q2 EPS Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS Previous is NaN for Q2 EPS Growth.\n\n")
            ...
            if sales_prev == np.nan:
                    logging.info(f"(Q2)Sales Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']} \n \
                                  Reason: Sales value in QuarterlyEPS where YearEndng={prev_year_qtr} (1 year before 2nd quarter) doesnt exist or has more than one value.\n\n")
            ...
            if sales_current == np.nan:
                    logging.info(f"(Q2)Sales Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']} \n \
                                  Reason: Sales value in QuarterlyEPS for YearEnding={prev_qtr} (2nd quarter) doesnt exist or has more than one value.\n\n")
            ...
            ##########################################
            if quarterly_eps_list.loc[index, 'Q2 Sales Growth'] == np.nan:
                logging.info(f"Q2 Sales Growth = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales Previous in QuarterlyEPS for the YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            #################  
    ...
    def consolidated_quarterly_two_eps_sales_growth(self, quarterly_eps_list, conn,today, logging):
        """Calculating the EPS and Sales Growth for current quarter,
        ...
        Return:
         ...
         """
        ##########################################
        logging.info("Consolidated Q EPS Sales Growth:")
        ##########################################
        qtr_two_start = self.get_previous_quarter(today).strftime("%Y-%m-%d")
        prev_year = self.get_year_before_quarter(self.get_previous_quarter(today))
        prev_2year =  self.get_year_before_quarter(prev_year).strftime("%Y-%m-%d")
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            if eps_current == np.nan:
                logging.info(f"(Q2)EPS Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS value in ConsolidatedQuarterlyEPS for YearEnding - {prev_qtr} (1 year before quarter) doesnt exist.\nConsidering Closest quarter.\n\n")
                ##########################################
            ...
            if sales_current == np.nan:
                logging.info(f"(Q2)Sales Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales value in ConsolidatedQuarterlyEPS for YearEnding - {prev_qtr} (1 year before quarter) doesnt exist.\nConsidering Closest quarter.\n\n")
                ##########################################
    ...
    def ttm_quarterly_list(self, date, conn, logging):
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            ######################################################
            if ttm_equity == np.nan:
                logging.info(f"TTM Equity is Null for {index} - row['CompanyCode'] {row["YearEnding"]} in QuarterlyEPS for year before quarter of {row["YearEnding"]}.")
            ######################################################
            ...
            ######################################################
            if ttm_calc.loc[index, 'EPS'] == np.nan:
                logging.info(f"EPS is Null for {index} - {row['CompanyCode']} {row["YearEnding"]} in QuarterlyEPS for year before quarter of {row["YearEnding"]}.")

            if ttm_calc.loc[index, 'NPM'] == np.nan:
                logging.info(f"NPM is Null for {index} - {row['CompanyCode']} {row["YearEnding"]} in QuarterlyEPS for year before quarter of {row["YearEnding"]}.")
            ######################################################
    ...
    def consolidated_ttm_quarterly_list(self, date, conn, logging):
        """ Generating the TTM data for newly available data in quarterly list,
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            ######################################################
            if ttm_equity == np.nan:
                logging.info(f"TTM Equity and TTM Last Equity is Null for {index} - row['CompanyCode'] {row["YearEnding"]} in ConsolidatedQuarterlyEPS for year before quarter of {row["YearEnding"]}.")
            ######################################################
            ...
            ######################################################
            if ttm_calc.loc[index, 'EPS'] == np.nan:
                logging.info(f"EPM is Null for {index} - row['CompanyCode'] {row["YearEnding"]} in QuarterlyEPS for year before quarter of {row["YearEnding"]}.")
            ######################################################
            if ttm_calc.loc[index, 'NPM'] == np.nan:
                logging.info(f"NPM is Null for {index} - row['CompanyCode'] {row["YearEnding"]} in QuarterlyEPS for year before quarter of {row["YearEnding"]}.")
            ######################################################
    ...
    def ttm_eps_sales_growth(self, quarterly_eps_list, today, conn, logging):
        ...
        ##########################################
        logging.info("TTM EPS Sales Growth:")
        ##########################################
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            ##########################################
            if eps_prev == np.nan:
                logging.info(f"EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in TTM where YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            eps_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == row["YearEnding"])]['EPS']
            eps_current = eps_current_list.item() if len(eps_current_list.index) == 1 else np.nan
            ##########################################
            if eps_current == np.nan:
                logging.info(f"EPS Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in TTM where YearEnding={row["YearEnding"]} doesnt exist or has more than one value.\n\n")
            ##########################################

            quarterly_eps_list.loc[index, 'TTM1 EPS'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev != 0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'TTM1 EPS'] == np.nan:
                    logging.info(f"TTM1 EPS = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS Previous is NaN for TTM1 EPS.\n\n")
            ##########################################
            ...
            ##########################################
            if sales_prev == np.nan:
                logging.info(f"(TTM1) Sales Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in TTM where YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            ...
            ##########################################
            if sales_current == np.nan:
                logging.info(f"(TTM1) Sales Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in TTM where YearEnding={row["YearEnding"]} doesnt exist or has more than one value.\n\n")
            ##########################################
            quarterly_eps_list.loc[index, 'TTM1 Sales'] = ((sales_current-sales_prev)/abs(sales_prev))*100 if sales_prev != 0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'TTM1 Sales'] == np.nan:
                    logging.info(f"TTM1 Sales = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales Previous is NaN for TTM1 Sales.\n\n")
            ##########################################
            ...
            ##########################################
            if eps_prev == np.nan:
                logging.info(f"(TTM 2)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in TTM where YearEnding={prev_two_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            eps_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_year_qtr)]['EPS']
            eps_current = eps_current_list.item() if len(eps_current_list.index) == 1 else np.nan
            ##########################################
            if eps_current == np.nan:
                logging.info(f"(TTM2)EPS Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in TTM where YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            quarterly_eps_list.loc[index, 'TTM2 EPS'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev != 0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'TTM2 EPS'] == np.nan:
                    logging.info(f"TTM2 EPS = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS Previous is NaN for TTM2 EPS.\n\n")
            ##########################################
            ...
            ##########################################
            if sales_prev == np.nan:
                logging.info(f"(TTM2) Sales Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in TTM where YearEnding={prev_two_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            sales_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_year_qtr)]['Sales']
            sales_current = sales_current_list.item() if len(sales_current_list.index) == 1 else np.nan		
            ##########################################
            if sales_current == np.nan:
                logging.info(f"(TTM2) Sales Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in TTM where YearEnding={prev_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            quarterly_eps_list.loc[index, 'TTM2 Sales'] = ((sales_current-sales_prev)/abs(sales_prev))*100 if sales_prev != 0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'TTM2 Sales'] == np.nan:
                    logging.info(f"TTM2 Sales = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales Previous is NaN for TTM2 Sales.\n\n")
            ##########################################
            if(sales_prev == 0 or sales_prev == np.nan):
            ...
            ##########################################
            if eps_prev == np.nan:
                logging.info(f"(TTM 3)EPS Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in TTM where YearEnding={prev_three_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            eps_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_two_year_qtr)]['EPS']
            eps_current = eps_current_list.item() if len(eps_current_list.index) == 1 else np.nan
            ##########################################
            if eps_current == np.nan:
                logging.info(f"(TTM3) EPS Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: EPS value in TTM where YearEnding={prev_two_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            quarterly_eps_list.loc[index, 'TTM3 EPS'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev != 0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'TTM3 EPS'] == np.nan:
                    logging.info(f"TTM3 EPS = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: EPS Previous is NaN for TTM3 EPS.\n\n")
            ##########################################
            ...
            ##########################################
            if sales_prev == np.nan:
                logging.info(f"(TTM3) Sales Prev = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in TTM where YearEnding={prev_three_year_qtr } doesnt exist or has more than one value.\n\n")
            ##########################################
            ...
            ##########################################
            if sales_current == np.nan:
                logging.info(f"(TTM3) Sales Current = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}\n \
                                  Reason: Sales value in TTM where YearEnding={prev_two_year_qtr} doesnt exist or has more than one value.\n\n")
            ##########################################
            quarterly_eps_list.loc[index, 'TTM3 Sales'] = ((sales_current-sales_prev)/abs(sales_prev))*100 if sales_prev != 0 else np.nan
            ##########################################
            if quarterly_eps_list.loc[index, 'TTM3 Sales'] == np.nan:
                    logging.info(f"TTM3 Sales = NaN\nIndex: {index}, CompanyCode: {row['CompanyCode']}, YearEnding: {row['YearEnding']}, EPS: {row['EPS']} \n \
                                  Reason: Sales Previous is NaN for TTM3 Sales.\n\n")
            ##########################################
            if(sales_prev == 0 or sales_prev == np.nan):
    ...
    def consolidated_ttm_eps_sales_growth(self, quarterly_eps_list, conn, logging):
        """ Generating the data of TTM, sales and EPS Growth,
    ...
    def get_all_ttm(self, quarterly_eps_list, conn, today, logging):
        ...
        ttm = self.ttm_quarterly_list(today, conn, logging)
        self.export_table("TTM", ttm, today)

        self.insert_ttm(ttm, conn)

    def get_all_consolidated_ttm(self, quarterly_eps_list, conn,today, logging):
        """ Fetch the all data of TTM,
        ...
        c_ttm = self.consolidated_ttm_quarterly_list(today, conn, logging)
    ...
    def eps_rating(self, quarterly_eps_list,conn,today, logging):
        ...
        quarterly_eps_list = self.ttm_eps_sales_growth(quarterly_eps_list, today, conn, logging)

        return quarterly_eps_list

        
    def consolidated_eps_rating(self, c_quarterly_eps_list,conn,today, logging):
        """ Calculating EPS Rating 
        ...
        c_quarterly_eps_list = self.consolidated_ttm_eps_sales_growth(c_quarterly_eps_list, conn, logging)
    ...
    def today_quarterly_eps(self, conn, cur,today, logging):
        ...
        if not(quarterly_eps_list.empty):
            ...
            quarterly_eps_list = self.quarterly_one_eps_sales_growth(quarterly_eps_list, conn, today, logging)
            print("exporting 1.1.1")
            ...
            quarterly_eps_list = self.quarterly_two_eps_sales_growth(quarterly_eps_list, conn, today, logging)
            ...
            self.get_all_ttm(quarterly_eps_list, conn,today, logging)
        
        else:
            print("Data not present for quarterly EPS list for ....Date: "+str(today))

        
    def consolidated_today_quarterly_eps(self, conn, cur,today, logging):
        """ generate the data for today quartely EPS,
        ...
        if not(c_quarterly_eps_list.empty):
            ...
            c_quarterly_eps_list = self.consolidated_quarterly_one_eps_sales_growth(c_quarterly_eps_list, conn,today, logging)
            ...
            c_quarterly_eps_list = self.consolidated_quarterly_two_eps_sales_growth(c_quarterly_eps_list, conn, today, logging)
            ...
            print("Calculating TTM values", flush = True)
            self.get_all_consolidated_ttm(c_quarterly_eps_list, conn,today, logging) # Consolidated flow added inside
    ...
    def history_eps_report(self, date, conn, cur):
        ...
        if not(quarterly_eps_list.empty):
            ...
            quarterly_eps_list = self.eps_rating(quarterly_eps_list, conn,today, logging)
    ...
    def current_eps_report(self, conn,cur,today, logging):
        ...
        self.today_quarterly_eps(conn, cur, today, logging)
        if(today >= datetime.date(2021, 4, 1)):
            self.consolidated_today_quarterly_eps(conn, cur, today, logging) 
        ...
        if not(quarterly_eps_list.empty):
            ...
            quarterly_eps_list = self.eps_rating(quarterly_eps_list, conn,today, logging)		#2
            ...
            if(today >= datetime.date(2021, 4, 1)):
                ...
                c_quarterly_eps_list = self.consolidated_eps_rating(c_quarterly_eps_list, conn,today, logging)
    ...
    def Generate_Daily_Report(self, curr_date, conn, cur):
        ...
        today = curr_date
        print("Gen EPS")
        logging.basicConfig(filename='logfile'+ str(today)+'.txt', level=logging.INFO, format='%(message)s')

        logging.info("\nEPS\n")     
        print("start")
        self.current_eps_report(conn,cur, today, logging)
