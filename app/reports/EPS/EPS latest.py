...
class EPS:
    ...
    def set_daily_qtr_eps(self, conn, today):
        ...
        WHERE QR."YearEnding" <= \'' + end_qtr + '\' and QR."YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\' AND QE."CompanyCode" is null;'
    # AND QR."ModifiedDate"<=\''+today.strftime("%Y-%m-%d")+'\';'
    ...
    def ttm_eps_sales_growth(self, quarterly_eps_list, today, conn):
        ...
        # raise Exception('Break for debugging')
            self.export_table('TTM EXPORT', quarterly_eps_list)
        return quarterly_eps_list
