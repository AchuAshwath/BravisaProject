...
class SMR:
	...
	def merge_background_industry_info(self, ratios_merge_list, conn):
		...
		ratios_merge_list = pd.merge(ratios_merge_list, industry_info[['IndustryCode', 'SubIndustry']], left_on = 'IndustryCode', \
		...
		for index, row in duplicates.iterrows():
			...
			if (duplicates_values['SubIndustry'] == 'Banks').all():
	...
	def insert_ratios_merge_list(self, ratios_merge_list, conn, cur, date):
		...
		ratios_merge_list_init = pd.DataFrame(columns = ['CompanyCode', 'CompanyName', 'SubIndustry', 'ROEYearEnding', 'Months', 'FaceValue', 'Sales Growth', 'NPM', \
		...
		# Renaming column
		ratios_merge_list = ratios_merge_list.rename(columns = {'YearEnding' : 'ROEYearEnding', 'SubIndustry' : 'Industry'})
