...
class EPS:
    ...
    # def get_closest_quarter(self, target):
    #     """Fetch the closest previous quarter end date from the target date."""
        
    #     # Define quarter end dates
    #     quarters = [datetime.date(target.year - 1, 12, 31),
    #                 datetime.date(target.year, 3, 31),
    #                 datetime.date(target.year, 6, 30),
    #                 datetime.date(target.year, 9, 30),
    #                 datetime.date(target.year, 12, 31)]

    #     # Check if target is exactly at the end of a quarter
    #     if target in quarters:
    #         # Find and return the previous quarter's end date
    #         target_index = quarters.index(target)
    #         return quarters[target_index - 1]
        
    #     # If not at the end of a quarter, find the closest previous quarter end
    #     for date in quarters[::-1]: # Iterate in reverse to find the previous quarter more efficiently
    #         if target > date:
    #             return date
    ...
    def set_daily_qtr_eps(self, conn, today):
        ...
        quarterly_list['E_ERS'] = quarterly_list['EBIDTA']/quarterly_list['Equity']
        ...
        'OPM', 'Tax', 'PATRAW', 'PAT', 'Equity', 'Reserves', 'EPS', 'NPM', 'Ext_Flag', 'E_ERS']]
    ...
    def insert_quarterly_eps_resulsts(self, quarterly_eps_list, conn):
        ...
        'OPM', 'Tax', 'PATRAW', 'PAT', 'Equity', 'Reserves', 'EPS', 'NPM', 'Ext_Flag', 'Q1 EPS Growth', 'Q1 Sales Growth', 'Q2 EPS', 'Q2 EPS Growth', 'Q2 Sales', 'Q2 Sales Growth', 'E_ERS']]
    ...
    def quarterly_one_eps_sales_growth(self, quarterly_eps_list, conn,today):
        ...
        sql = 'SELECT * FROM public."QuarterlyEPS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_4year + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
    ...
    def consolidated_quarterly_one_eps_sales_growth(self, quarterly_eps_list, conn, today):
        ...
        sql = 'SELECT * FROM public."ConsolidatedQuarterlyEPS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_4year + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        quarterly_eps_year_back = sqlio.read_sql_query(sql, con = conn)	
        quarterly_eps_list['Sales'] =quarterly_eps_list['Sales'].replace(r'[?$,]', '', regex=True).astype(float)
        quarterly_eps_year_back['Sales'] = quarterly_eps_year_back['Sales'].replace(r'[?$,]', '', regex=True).astype(float)
    ...
    def quarterly_two_eps_sales_growth(self, quarterly_eps_list, conn,today):
        ...
        sql = 'SELECT * FROM public."QuarterlyEPS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_2year + '\' AND "YearEnding"<= \'' + today.strftime("%Y-%m-%d") + '\';'
        ...
        self.export_table("EPS LIST", quarterly_eps_list)
        return quarterly_eps_list
    ...
    def consolidated_quarterly_two_eps_sales_growth(self, quarterly_eps_list, conn,today):
        ...
        sql = 'SELECT * FROM public."ConsolidatedQuarterlyEPS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_2year + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        quarterly_eps_year_back = pd.read_sql_query(sql, con = conn)
    ...
    def ttm_quarterly_list(self, date, conn):
        ...
        qeps_back = 'SELECT * FROM public."QuarterlyEPS" WHERE "YearEnding" >= \''+prev_4year.strftime("%Y-%m-%d")+'\' AND "YearEnding"<= \'' + date.strftime("%Y-%m-%d") + '\';'
        ...
        'PAT', 'Equity',  'Reserves', 'Months', 'Quarter', 'EPS', 'NPM', 'E_ERS'])
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            ttm_calc.loc[index, 'E_ERS'] = ttm_calc.loc[index, 'EBIDTA']/ttm_calc.loc[index, 'Equity'] if ttm_calc.loc[index, 'Equity'] != 0 else np.nan
        ...
        'PAT', 'Equity', 'Reserves', 'EPS', 'NPM', 'E_ERS']]
    ...
    def consolidated_ttm_quarterly_list(self, date, conn):
        ...
        qeps_back = 'SELECT * FROM public."ConsolidatedQuarterlyEPS" WHERE "YearEnding" >= \''+prev_4year.strftime("%Y-%m-%d")+'\' and "YearEnding" <= \'' + date.strftime("%Y-%m-%d") + '\';'
        qrtlylist = sqlio.read_sql_query(qeps_back, con = conn)
    ...
    def ttm_eps_sales_growth(self, quarterly_eps_list, today, conn):
        ...
        sql = 'SELECT * from public."TTM" WHERE "YearEnding" >= \'' + five_year_back + '\' AND "YearEnding"<= \'' + today.strftime("%Y-%m-%d") + '\';'
    ...
    def consolidated_ttm_eps_sales_growth(self, quarterly_eps_list, today, conn):
        ...
        sql = 'SELECT * from public."ConsolidatedTTM" WHERE "YearEnding" >= \'' + five_year_back + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        ttm_backyear = pd.read_sql_query(sql, con = conn)
    ...
    def insert_sa_EPS(self, name,table, conn, cur, today):
        ...
        # raise Exception("Break for testing")
        copy_sql = """
                COPY "Reports"."STANDALONE_EPS" FROM stdin WITH CSV HEADER
                DELIMITER as ','
                """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
    ...
    def get_eps_list(self, conn, today):
        ...
        eps_df = sqlio.read_sql_query('SELECT * FROM public."QuarterlyEPS" WHERE "YearEnding" <= \'' + today + '\';', con=conn)  # Fetch all data from QuarterlyEPS table
        self.export_table("BTTDF", btt_df)
        self.export_table("EPSDF", eps_df)
    ...
    def get_consolidated_eps_list(self, conn, today):
        ...
        # Fetch data from both tables
        btt_df = sqlio.read_sql_query('SELECT * FROM public."BTTList" WHERE "BTTDate" >= %(btt_back)s AND "BTTDate" < %(btt_next)s;', con=conn, params={'btt_back': btt_back, 'btt_next': btt_next})
        eps_df = sqlio.read_sql_query('SELECT * FROM public."ConsolidatedQuarterlyEPS" WHERE "YearEnding" <= \'' + today + '\';', con=conn)  # Fetch all data from QuarterlyEPS table
        self.export_table("BTTDF", btt_df)
        self.export_table("EPSDF", eps_df)
        # Filter rows in eps_df based on conditions
        eps_df = eps_df[eps_df['YearEnding'] == eps_df.groupby('CompanyCode')['YearEnding'].transform('max')]  # Filter based on max YearEnding
    ...
    #     quarterly_eps_list = self.get_eps_list(conn,today)		#1
    ...
    def current_cons_eps_report(self, conn,cur,today):
        ...
        if not(c_quarterly_eps_list.empty):
            ...
            self.insert_Consolidated_EPS("EPS_cons_" + today.strftime("%Y-%m-%d"), c_quarterly_eps_list, conn, cur,today)
            return return_list
