...
class ERS:
    ...
    def set_daily_qtr_eps(self, conn, today):
        ...
        sql = 'SELECT QR.* from public."QuarterlyResults" QR LEFT JOIN public."QuarterlyERS" QE on QR."CompanyCode"= QE."CompanyCode" and QE."YearEnding" = QR."YearEnding"\
        ...
        quarterly_list['ERS'] = quarterly_list['PAT']/quarterly_list['Equity']
        ...
        'OPM', 'Tax', 'PATRAW', 'PAT', 'Equity', 'Reserves', 'ERS', 'NPM', 'Ext_Flag']]


        quarterly_eps_list = pd.concat([quarterly_eps_list, pd.DataFrame(columns = ['Q1 ERS Growth','Q1 Sales Growth', 'Q2 ERS', 'Q2 ERS Growth', 'Q2 Sales','Q2 Sales Growth'])], sort=False)
    ...
    def consolidated_set_daily_qtr_eps(self, conn, today):
        ...
        print("End Qtr: ", end_qtr)
        # print("START QTR: {} END QTR: {}".format(start_qtr, end_qtr))
        sql = 'SELECT QR.* from public."ConsolidatedQuarterlyResults" QR LEFT JOIN public."ConsolidatedQuarterlyERS" QE on QR."CompanyCode"= QE."CompanyCode" and QE."YearEnding" = QR."YearEnding"\
        WHERE QR."YearEnding" <= \'' + end_qtr + '\' AND QE."CompanyCode" is null;'
        # AND QR."ModifiedDate"<=\''+str(today + datetime.timedelta(1))+'\';'
    ...
    def insert_quarterly_eps_resulsts(self, quarterly_eps_list, conn):
        ...
        'OPM', 'Tax', 'PATRAW', 'PAT', 'Equity', 'Reserves', 'ERS', 'NPM', 'Ext_Flag', 'Q1 ERS Growth', 'Q1 Sales Growth', 'Q2 ERS', 'Q2 ERS Growth', 'Q2 Sales', 'Q2 Sales Growth']]

        exportfilename = "QuarterlyERSListExport.csv"
        exportfile = open(exportfilename,"w+")
        quarterly_eps_list.to_csv(exportfile, header=True, index=False, lineterminator='\r')
        exportfile.close()

        copy_sql = """
            COPY "public"."QuarterlyERS" FROM stdin WITH CSV HEADER
            DELIMITER as ','
            """
        ...
        updatequery = 'UPDATE public."QuarterlyERS" SET "Ext_Flag" = true WHERE "Ext_Flag" IS NULL'
    ...
    def insert_consolidated_quarterly_eps_results(self, quarterly_eps_list, conn):
        ...
        exportfilename = "ConsolidatedQuarterlyERSListExport.csv"
        exportfile = open(exportfilename,"w+")
        quarterly_eps_list.to_csv(exportfile, header=True, index=False, lineterminator='\r')
        exportfile.close()

        copy_sql = """
            COPY "public"."ConsolidatedQuarterlyERS" FROM stdin WITH CSV HEADER
            DELIMITER as ','
            """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
        ...
        updatequery = 'UPDATE public."ConsolidatedQuarterlyERS" SET "Ext_Flag" = true WHERE "Ext_Flag" IS NULL'
    ...
    def quarterly_one_eps_sales_growth(self, quarterly_eps_list, conn,today):
        ...
        sql = 'SELECT * FROM public."QuarterlyERS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_4year + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            eps_prev_list = quarterly_eps_year_back.loc[(quarterly_eps_year_back["CompanyCode"]==row['CompanyCode']) & (quarterly_eps_year_back["YearEnding"] == prev_year_qtr)]["ERS"]
            ...
            if (math.isnan(eps_prev)):
                eps_prev_list = quarterly_eps_list.loc[(quarterly_eps_list["CompanyCode"]==row['CompanyCode']) & (quarterly_eps_list["YearEnding"] == prev_year_qtr)]["ERS"]
                if len(eps_prev_list.index) > 1:
                    eps_prev = eps_prev_list[eps_prev_list.index[0]]
                else:
                    eps_prev = eps_prev_list.item() if len(eps_prev_list.index) != 0 else np.nan
                ...
                quarterly_eps_list.loc[index, 'Q1 ERS Growth'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev  !=0 else np.nan
    ...
    def consolidated_quarterly_one_eps_sales_growth(self, quarterly_eps_list, conn, today):
        ...
        sql = 'SELECT * FROM public."ConsolidatedQuarterlyERS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_4year + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        quarterly_eps_year_back = sqlio.read_sql_query(sql, con = conn)	
        quarterly_eps_list['Sales'] =quarterly_eps_list['Sales'].replace(r'[?$,]', '', regex=True).astype(float)
        quarterly_eps_year_back['Sales'] = quarterly_eps_year_back['Sales'].replace(r'[?$,]', '', regex=True).astype(float)
    ...
    def quarterly_two_eps_sales_growth(self, quarterly_eps_list, conn,today):
        ...
        sql = 'SELECT * FROM public."QuarterlyERS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_2year + '\' AND "YearEnding"<= \'' + today.strftime("%Y-%m-%d") + '\';'
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            eps_current_list = quarterly_eps_year_back.loc[(quarterly_eps_year_back["CompanyCode"]==row['CompanyCode']) & (quarterly_eps_year_back["YearEnding"] == prev_qtr)]['ERS']
            eps_current = eps_current_list.item() if len(eps_current_list.index) == 1 else np.nan
                        
            quarterly_eps_list.loc[index, 'Q2 ERS Growth'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev  !=0 else np.nan
            quarterly_eps_list.loc[index , 'Q2 ERS'] = eps_current
        ...
        self.export_table("ERS LIST", quarterly_eps_list)
        return quarterly_eps_list
    ...
    def consolidated_quarterly_two_eps_sales_growth(self, quarterly_eps_list, conn,today):
        ...
        sql = 'SELECT * FROM public."ConsolidatedQuarterlyERS" where "Ext_Flag" is not null and "YearEnding" >= \'' + prev_2year + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        quarterly_eps_year_back = pd.read_sql_query(sql, con = conn)
    ...
    def ttm_quarterly_list(self, date, conn):
        ...
        sql = 'SELECT QE.* from public."QuarterlyERS" QE LEFT JOIN public."ERS_TTM" TTM on QE."CompanyCode"= TTM."CompanyCode" and TTM."YearEnding" = QE."YearEnding"\
        ...
        qeps_back = 'SELECT * FROM public."QuarterlyERS" WHERE "YearEnding" >= \''+prev_4year.strftime("%Y-%m-%d")+'\' AND "YearEnding"<= \'' + date.strftime("%Y-%m-%d") + '\';'
        ...
        'PAT', 'Equity',  'Reserves', 'Months', 'Quarter', 'ERS', 'NPM'])
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            # f_eps.write(line)
            ttm_calc.loc[index, 'ERS'] = ttm_calc.loc[index, 'PAT']/ttm_calc.loc[index, 'Equity'] if ttm_calc.loc[index, 'Equity'] != 0 else np.nan
    ...
    def consolidated_ttm_quarterly_list(self, date, conn):
        ...
        sql = 'SELECT QE.* from public."ConsolidatedQuarterlyERS" QE LEFT JOIN public."ConsolidatedERSTTM" TTM on QE."CompanyCode"= TTM."CompanyCode" and TTM."YearEnding" = QE."YearEnding"\
        WHERE QE."YearEnding" <= \'' + qtr_now.strftime("%Y-%m-%d") + '\' AND TTM."CompanyCode" is null AND QE."Ext_Flag" is not null;'
        quarterly_eps_list = sqlio.read_sql_query(sql, con = conn)

        qeps_back = 'SELECT * FROM public."ConsolidatedQuarterlyERS" WHERE "YearEnding" >= \''+prev_4year.strftime("%Y-%m-%d")+'\' and "YearEnding" <= \'' + date.strftime("%Y-%m-%d") + '\';'
        qrtlylist = sqlio.read_sql_query(qeps_back, con = conn)
    ...
    def ttm_eps_sales_growth(self, quarterly_eps_list, today, conn):
        ...
        sql = 'SELECT * from public."ERS_TTM" WHERE "YearEnding" >= \'' + five_year_back + '\' AND "YearEnding"<= \'' + today.strftime("%Y-%m-%d") + '\';'
        ttm_backyear = pd.read_sql_query(sql, con = conn)
        # self.export_table(ttm_backyear, 'ttm_backyear')
        # raise Exception("Break")
        quarterly_eps_list = pd.concat([quarterly_eps_list, pd.DataFrame(columns = ['TTM1 ERS', 'TTM1 Sales', 'TTM2 ERS', 'TTM2 Sales',\
        'TTM3 ERS', 'TTM3 Sales'])], sort=False)
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            prev_year_qtr = self.get_year_before_quarter(row['YearEnding'])
            eps_prev_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_year_qtr)]['ERS']
            eps_prev = eps_prev_list.iloc[0] if len(eps_prev_list.index) >= 1 else np.nan
            
            eps_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == row["YearEnding"])]['ERS']
            eps_current = eps_current_list.iloc[0] if len(eps_current_list.index) >= 1 else np.nan

            # ^^^^^

            quarterly_eps_list.loc[index, 'TTM1 ERS'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev != 0 else np.nan
            ...
            ttm_eps_val = quarterly_eps_list.loc[index, 'TTM1 ERS'] 
            ttm_eps_set_val = ( ((0.3)* ttm_eps_val) if not math.isnan(ttm_eps_val) else 0 ) 
            quarterly_eps_list.loc[index, 'ERS Rating'] = quarterly_eps_list.loc[index, 'ERS Rating'] + ttm_eps_set_val
            ...
            eps_prev_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_two_year_qtr)]['ERS']
            eps_prev = eps_prev_list.iloc[0] if len(eps_prev_list.index) >= 1 else np.nan
            
            eps_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_year_qtr)]['ERS']
            eps_current = eps_current_list.iloc[0] if len(eps_current_list.index) >= 1 else np.nan

            quarterly_eps_list.loc[index, 'TTM2 ERS'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev != 0 else np.nan
            ...
            ttm_eps_val = quarterly_eps_list.loc[index, 'TTM2 ERS'] 
            ...
            eps_prev_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_three_year_qtr)]['ERS']
            eps_prev = eps_prev_list.iloc[0] if len(eps_prev_list.index) >= 1 else np.nan
            
            eps_current_list = ttm_backyear.loc[(ttm_backyear["CompanyCode"]==row['CompanyCode']) & (ttm_backyear["YearEnding"] == prev_two_year_qtr)]['ERS']
            eps_current = eps_current_list.iloc[0] if len(eps_current_list.index) >= 1 else np.nan

            quarterly_eps_list.loc[index, 'TTM3 ERS'] = ((eps_current-eps_prev)/abs(eps_prev))*100 if eps_prev != 0 else np.nan
            ...
            ttm_eps_val = (0.1) * quarterly_eps_list.loc[index, 'TTM3 ERS'] 
    ...
    def consolidated_ttm_eps_sales_growth(self, quarterly_eps_list, today, conn):
        ...
        sql = 'SELECT * from public."ConsolidatedERSTTM" WHERE "YearEnding" >= \'' + five_year_back + '\' and "YearEnding" <= \'' + today.strftime("%Y-%m-%d") + '\';'
        ttm_backyear = pd.read_sql_query(sql, con = conn)
    ...
    def insert_ttm(self, ttm, conn):
        ...
        copy_sql = """
                COPY "public"."ERS_TTM" FROM stdin WITH CSV HEADER
                DELIMITER as ','
                """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
    ...
    def insert_c_ttm(self, ttm, conn):
        ...
        copy_sql = """
                COPY "public"."ConsolidatedERSTTM" FROM stdin WITH CSV HEADER
                DELIMITER as ','
                """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
    ...
    def get_ttm(self, today, conn):
        ...
        sql = 'SELECT * FROM public."ERS_TTM" where "YearEnding" <=\''+self.get_closest_quarter(today).strftime("%Y-%m-%d")+'\';'
    ...
    def eps_rating(self, quarterly_eps_list,conn,today):
        ...
        for index, row in quarterly_eps_list.iterrows():
            ...
            Q1_EPS_Growth = (0.2)*quarterly_eps_list.loc[index, 'Q1 ERS Growth']
            Q2_EPS_Growth = (0.2)*quarterly_eps_list.loc[index, 'Q2 ERS Growth']

            quarterly_eps_list.loc[index, 'ERS Rating'] = (Q1_EPS_Growth if not (math.isnan(Q1_EPS_Growth) or Q1_EPS_Growth == np.nan)  else 0) +\
    ...
    def consolidated_eps_rating(self, c_quarterly_eps_list,conn,today):
        ...
        for index, row in c_quarterly_eps_list.iterrows():
            if(c_quarterly_eps_list.loc[index, 'Q1 ERS Growth'] is not None):
                Q1_EPS_Growth = (0.2)*c_quarterly_eps_list.loc[index, 'Q1 ERS Growth']
            else:
                Q1_EPS_Growth = np.nan
            if(c_quarterly_eps_list.loc[index, 'Q2 ERS Growth'] is not None):
                Q2_EPS_Growth = (0.2)*c_quarterly_eps_list.loc[index, 'Q2 ERS Growth']
            else:
                Q2_EPS_Growth = np.nan

            c_quarterly_eps_list.loc[index, 'ERS Rating'] = (Q1_EPS_Growth if not (math.isnan(Q1_EPS_Growth) or Q1_EPS_Growth == np.nan)  else 0) +\
                                                        (Q2_EPS_Growth if not(math.isnan(Q2_EPS_Growth) or Q2_EPS_Growth == np.nan) else 0)
    ...
    def stock_percentile_ranking(self, quarterly_eps_list):
        ...
        quarterly_eps_list['ERS Ranking'] = quarterly_eps_list['ERS Rating'].rank(ascending=False)
        quarterly_eps_list['ERS Ranking'] = ((len(quarterly_eps_list.index)-quarterly_eps_list['ERS Ranking']+1)/len(quarterly_eps_list.index))*100
    ...
    def insert_EPS(self, name,table, conn, cur, today):
        ...
        table["ERS Date"] = today.strftime("%Y-%m-%d")
        ...
        table = table.rename(columns = {'Sales' : 'Q1 Sales', 'ERS': 'Q1 ERS'})
        # Creating table
        table = table[['CompanyCode', 'NSECode', 'BSECode', 'CompanyName', 'ISIN', 'Months', 'Quarter', 'YearEnding', \
        'Q1 ERS', 'Q1 ERS Growth' , 'Q1 Sales','Q1 Sales Growth' , 'Q2 ERS', 'Q2 ERS Growth', 'Q2 Sales','Q2 Sales Growth', \
        'TTM1 ERS', 'TTM1 Sales', 'TTM2 ERS', 'TTM2 Sales', 'TTM3 ERS', 'TTM3 Sales', 'NPM', 'ERS Rating', 'ERS Ranking', 'ERS Date']]
        ...
        copy_sql = """
                COPY "Reports"."ERS" FROM stdin WITH CSV HEADER
                DELIMITER as ','
                """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
    ...
    def insert_sa_EPS(self, name,table, conn, cur, today):
        ...
        copy_sql = """
                COPY "Reports"."STANDALONE_ERS" FROM stdin WITH CSV HEADER
                DELIMITER as ','
                """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
    ...
    def insert_Consolidated_EPS(self, name,table, conn, cur, today):
        ...
        copy_sql = """
                COPY "Reports"."Consolidated_ERS" FROM stdin WITH CSV HEADER
                DELIMITER as ','
                """
        with open(exportfilename, 'r') as f:
            cur.copy_expert(sql=copy_sql, file=f)
    ...
    def get_eps_list(self, conn, today):
        ...
        eps_df = sqlio.read_sql_query('SELECT * FROM public."QuarterlyERS" WHERE "YearEnding" <= \'' + today + '\';', con=conn)  # Fetch all data from QuarterlyEPS table
        self.export_table("BTTDF", btt_df)
        self.export_table("ERSDF", eps_df)
        ...
        self.export_table("GET ERS LIST", merged_df)
        return merged_df
    ...
    def get_consolidated_eps_list(self, conn, today):
        ...
        # Fetch data from both tables
        btt_df = sqlio.read_sql_query('SELECT * FROM public."BTTList" WHERE "BTTDate" >= %(btt_back)s AND "BTTDate" < %(btt_next)s;', con=conn, params={'btt_back': btt_back, 'btt_next': btt_next})
        eps_df = sqlio.read_sql_query('SELECT * FROM public."ConsolidatedQuarterlyERS" WHERE "YearEnding" <= \'' + today + '\';', con=conn)  # Fetch all data from QuarterlyEPS table
        self.export_table("BTTDF", btt_df)
        self.export_table("ERSDF", eps_df)
        # Filter rows in eps_df based on conditions
        eps_df = eps_df[eps_df['YearEnding'] == eps_df.groupby('CompanyCode')['YearEnding'].transform('max')]  # Filter based on max YearEnding
    ...
    def current_sa_eps_report(self, conn,cur,today):
        ...
        if not(quarterly_eps_list.empty):
            ...
            self.insert_sa_EPS("EPS_SA_" + today.strftime("%Y-%m-%d"), quarterly_eps_list, conn, cur,today)
