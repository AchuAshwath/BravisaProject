...
class SMR:
	...
	def compile_ratios_list_current(self, conn, date):
		...
		sql_ratios_banking = """
								SELECT DISTINCT ON ("CompanyCode") rb.*
								FROM public."RatiosBankingVI" rb
								WHERE rb."YearEnding" <= DATE('{}')
								ORDER BY rb."CompanyCode", rb."YearEnding" DESC;
							""".format(date)
		...
		sql_ratios_nonbanking = """
									SELECT DISTINCT ON ("CompanyCode") rnb.*
									FROM public."RatiosNonBankingVI" rnb
									WHERE rnb."YearEnding" <= DATE('{}')
									ORDER BY rnb."CompanyCode", rnb."YearEnding" DESC;
								""".format(date)
	...
	def compile_sales_npm_roe(self, ratios_merge_list, date, conn):
		...
		sql_ttm = """
					...
					FROM public."TTM" ttm
					WHERE ttm."YearEnding" <= DATE('{}')
					ORDER BY ttm."CompanyCode", ttm."YearEnding" DESC;
				""".format(str(date))
		...
		ttm_list = sqlio.read_sql_query(sql_ttm, con = conn)
		...
		ttm_year_back_list =  sqlio.read_sql_query(sql_ttm_year_back, con = conn)
	...
	def get_bttlist_stocks_current(self, conn,today):
		...
		LEFT JOIN public."RatiosMergeList" RM \
			ON RM."CompanyCode" = BTT."CompanyCode" \
			AND RM."GenDate"= (SELECT max("GenDate") FROM public."RatiosMergeList"  WHERE "CompanyCode"=BTT."CompanyCode" AND "GenDate" <= DATE(\''+ str(today) + '\')) \
		WHERE "BTTDate">= \'' + BTT_back + '\' and "BTTDate"< \'' + BTT_next + '\';'
